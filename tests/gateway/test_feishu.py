...
class TestGroupMentionAtAll(unittest.TestCase):
    ...
    @patch.dict(os.environ, {"FEISHU_GROUP_POLICY": "open"}, clear=True)
    def test_at_all_in_content_is_ignored_without_explicit_bot_mention(self):
        from gateway.config import PlatformConfig
        from plugins.platforms.feishu.adapter import FeishuAdapter
    ...
    def test_at_all_is_ignored_even_for_allowlisted_user(self):
        """@_all targets the group, not Hermes, so it stays silent."""
        from gateway.config import PlatformConfig
        from plugins.platforms.feishu.adapter import FeishuAdapter
        ...
        # Allowlisted user is still blocked because the message targets @all.
